...
def run_demo():
    ...
    while not rospy.is_shutdown():
        ...
        if distance < 1:
            ...
            velocity = 0
            if sonard > 4 and distance>robot.getDistanceToFlag():
                pass
            elif sonard>0 and sonard<4:
                prevp = robot.get_robot_pose()
                velocity = 0
                angle = 0.1
                
            else:
                velocity = 2
                currp = robot.get_robot_pose()
                if currp == 0:
                    pass
                else: 
                    if prevp - currp[2] > 0.01:     
                        velocity = 0
                        angle = -0.1
                    elif prevp - currp[2] < -0.01:
                        velocity = 0
                        angle = 0.1
                    else:
                        angle = 0
                ...
                currentp = robot.get_robot_pose()
                distance = robot.getDistanceToFlag()
                #currentd = math.sqrt(currentp[0]**2+currentp[1]**2)
                print("current pose: ", currentp)
                print("flag position: ", distance)
